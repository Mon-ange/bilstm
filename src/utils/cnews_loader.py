...
def read_category():
    """读取分类目录，固定"""
    with open('data/labels.txt','r',encoding='utf8') as f:
...
def to_words(content, words):
    """将id表示的内容转换为文字"""
    return ''.join(words[x] for x in content if x != 0)
...
def process_file(filename, word_to_id, cat_to_id,max_length):
    ...
    for i in range(len(contents)):
        ...
        # if len(contents[i]) > max_length:
        #     max_length = len(contents[i])

    # 使用keras提供的pad_sequences来将文本pad为固定长度
    x_pad = kr.preprocessing.sequence.pad_sequences(data_id, max_length)
    y_pad = label_id  # 将标签转换为one-hot表示
